...
void App::setBackground(const string& themedir) {
	...
	if (loaded) {
		...
		if (bgstyle == "stretch") {
            if(nsizes > 0){
                image->Resize(pXRRScreenSize->width, pXRRScreenSize->height);
            }else{
                image->Resize(XWidthOfScreen(ScreenOfDisplay(Dpy, Scr)), XHeightOfScreen(ScreenOfDisplay(Dpy, Scr)));
            }
		} else if (bgstyle == "tile") {
            if(nsizes > 0){
                image->Tile(pXRRScreenSize->width, pXRRScreenSize->height);
            }else{
            ...
            if(nsizes > 0){
                image->Center(pXRRScreenSize->width, pXRRScreenSize->height, hexvalue.c_str());
            }else{
                image->Center(XWidthOfScreen(ScreenOfDisplay(Dpy, Scr)), XHeightOfScreen(ScreenOfDisplay(Dpy, Scr)), hexvalue.c_str());
            }
