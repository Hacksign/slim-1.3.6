...
Panel::Panel(Display* dpy, int scr, Window root, Cfg* config,
    ...
    int nsizes = 0;
    XRRScreenSize *pXRRScreenSize = XRRSizes(Dpy, Scr, &nsizes);
	if (mode == Mode_Lock) {
		if (bgstyle == "stretch")
		...
		if (bgstyle == "stretch") {
            if(nsizes > 0){
                bg->Resize(pXRRScreenSize->width, pXRRScreenSize->height);
            }else{
                bg->Resize(XWidthOfScreen(ScreenOfDisplay(Dpy, Scr)),
                            XHeightOfScreen(ScreenOfDisplay(Dpy, Scr)));
            }
            ...
            if(nsizes > 0){
                bg->Tile(pXRRScreenSize->width, pXRRScreenSize->height);
            }else{
			...
			hexvalue = hexvalue.substr(1,6);
            if(nsizes > 0){
                bg->Center(pXRRScreenSize->width, pXRRScreenSize->height, hexvalue.c_str());
            }else{
	...
	if (mode == Mode_Lock) {
        ...
        if(nsizes > 0){
            X = Cfg::absolutepos(cfgX, pXRRScreenSize->width, image->Width());
            Y = Cfg::absolutepos(cfgY, pXRRScreenSize->height, image->Height());
        }else{
...
void Panel::WrongPassword(int timeout) {
    ...
    int nsizes = 0, msg_x, msg_y;
    XRRScreenSize *pXRRScreenSize = XRRSizes(Dpy, Scr, &nsizes);
    if(nsizes > 0){
        msg_x = Cfg::absolutepos(cfgX, pXRRScreenSize->width, extents.width);
        msg_y = Cfg::absolutepos(cfgY, pXRRScreenSize->height, extents.height);
    }else{
...
void Panel::ShowSession() {
    ...
    XRRScreenSize *pXRRScreenSize = XRRSizes(Dpy, Scr, &nsizes);
    if(nsizes > 0){
        x = Cfg::absolutepos(msg_x, pXRRScreenSize->width, extents.width);
        y = Cfg::absolutepos(msg_y, pXRRScreenSize->height, extents.height);
    }else{
